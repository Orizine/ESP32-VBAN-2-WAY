...
void recvMsg(uint8_t *data, size_t len){
  ...
  if (d.startsWith("vol")) 
    {int newvol = ((d.charAt(3))-48);
      if (newvol>=0 && newvol<=9){volume = (1/(newvol+1)) ; SetVolume();} 
      else {WebSerial.println("volume incorect. entre 0 et 4");}
    } 
  else if (d.startsWith("reset")) { ESP.restart();}     
  else{ WebSerial.println("Commande incorrecte");}
}
